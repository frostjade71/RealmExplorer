...
export function CategoryBadge({ category, className }: { category: ServerCategory, className?: string }) {
  ...
  return (
    <div className={twMerge(clsx("flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[10px] font-headline font-bold uppercase tracking-wider w-fit", style.bg, style.text, className))}>
      {style.icon}
      <span>{category}</span>
    </div>
  )
