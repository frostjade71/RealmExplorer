...
export function RoleBadge({ role, className }: { role: UserRole, className?: string }) {
  return (
    <span className={twMerge(
      clsx(
        "inline-flex items-center px-1.5 py-0.5 font-pixel text-[7px] uppercase tracking-widest backdrop-blur-md",
      ...
      {role === 'explorer+' && (
        <img src={goldIngot} alt="" className="w-3 h-3 mr-1 object-contain" />
      )}
      {role === 'explorer' && (
        <img src={ironIngot} alt="" className="w-3 h-3 mr-1 object-contain" />
      )}
      {role === 'admin' && (
        <img src={adminIcon} alt="" className="w-3 h-3 mr-1 object-contain" />
      )}
      {role === 'moderator' && (
        <img src={modIcon} alt="" className="w-3 h-3 mr-1 object-contain" />
      )}
