...
import { LoadingSpinner, TopLoadingBar } from '../components/FeedbackStates'
import errorImage from '../assets/error/teto-but-re.webp'
import { CategoryBadge } from '../components/CategoryBadge'
import { Globe, Copy, CheckCircle, ArrowUpSquare, Star, ExternalLink, Calendar, Clock, Flag, Mail, ChevronLeft, ChevronRight, Activity, Users, Trophy, Gift, Info, RefreshCw, Share2, FileText, TrendingUp, Crown, Eye, Bookmark, MoreVertical, Maximize2, Pencil } from 'lucide-react'
import { formatDistanceToNow } from 'date-fns'
import { SiDiscord, SiTiktok, SiInstagram, SiYoutube, SiFacebook, SiTwitch, SiGithub, SiX, SiPatreon, SiKofi } from 'react-icons/si'
...
const VoteTimer = lazy(() => import('../components/VoteTimer').then(module => ({ default: module.VoteTimer })))
const RatingModal = lazy(() => import('../components/RatingModal').then(module => ({ default: module.RatingModal })))
...
const ServerPlayerChart = lazy(() => import('../components/ServerPlayerChart'))
...
export function ServerDetailPage() {
  ...
  const { user, isAdmin, isModerator, signInWithDiscord } = useAuth()
  ...
  const { data: liveStatus, isLoading: isLiveLoading, isError: isLiveError } = useLiveServerStatus(server, activeIpType)
  ...
  const handleCopyIp = () => {
    if (server?.ip_or_code) {
      ...
      setCopied(true)
      toast.success('IP Copied!', { description: `${fullIp} copied to clipboard` })
  ...
  const handleCopyBedrockIp = () => {
    if (server?.bedrock_ip) {
      ...
      toast.success('Bedrock IP Copied!', { description: `${fullIp} copied to clipboard` })
  ...
  // Security check: Only owners and admins can view unapproved servers
  const isOwner = user?.id && server?.owner_id && user.id === server.owner_id
  const hasAccess = !server || isApproved || isOwner || isAdmin || isModerator
        ...
        <div className="text-center space-y-4 animate-in fade-in zoom-in duration-500 p-4 flex flex-col items-center">
          ...
          <div className="space-y-2 px-4">
            ...
            <p className="text-xs md:text-sm text-on-surface-variant font-body max-w-sm mx-auto">
              This server is currently under review or has been made private.
            </p>
          </div>
          <div className="pt-4">
            <Link 
  ...
  return (
    ...
    <AnimatedPage className="max-w-5xl mx-auto w-full px-4 md:px-8 py-8 md:py-12">
      {/* Banner */}
      <FramerIn delay={0.1} className={`w-full h-32 md:h-64 bg-zinc-950 rounded-t-xl overflow-hidden relative ${isPremium ? 'border-t-2 border-x-2 border-[#f2a929]' : 'border-t border-x border-zinc-800'}`}>
        <div className="w-full h-full relative" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
          {server.banner_url ? (
             <motion.img 
              initial={isMobile ? { opacity: 0.8 } : { scale: 1.05, opacity: 0 }}
              animate={isMobile ? { opacity: 0.8 } : { scale: 1.1, opacity: 0.8 }}
              style={isMobile ? undefined : { x: springX, y: springY }}
              transition={{ duration: 0.8 }}
              src={server.banner_url} 
              alt="Banner" 
              className="w-full h-full object-cover will-change-[opacity,transform] absolute inset-0" 
              fetchPriority="high"
              loading="eager"
              decoding="sync"
             />
      ...
      {/* Header Info */}
      <div className={`bg-zinc-950 rounded-b-xl p-5 md:p-8 mb-4 md:mb-4 flex flex-col md:flex-row gap-4 md:gap-6 items-start relative z-30 -mt-4 will-change-transform ${isPremium ? 'border-x-2 border-b-2 border-[#f2a929]' : 'border-x border-b border-zinc-800 shadow-xl'}`}>
        <div className="relative -mt-10 md:-mt-12 z-10 flex-shrink-0">
          <div className={`w-20 h-20 md:w-24 md:h-24 bg-zinc-900 rounded-xl overflow-hidden border-4 ${isPremium ? 'border-transparent' : 'border-zinc-950'} shadow-lg will-change-transform relative z-10`}>
            {server.icon_url ? (
              <img src={server.icon_url} alt="Icon" fetchPriority="high" loading="eager" className="w-full h-full object-cover" />
            ) : (
            ...
            <div className="min-w-0 md:flex-1 w-full md:w-auto">
              ...
              <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-2 md:mb-4">
                ...
                <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[9px] md:text-[10px] font-headline font-bold uppercase tracking-wider w-fit bg-zinc-800/50 text-zinc-400 border border-zinc-700/50">
                  <img src={server.type === 'server' ? serverTypeIcon : realmTypeIcon} alt="" fetchPriority="high" loading="eager" className="w-3 md:w-3.5 h-3 md:h-3.5 object-contain rounded-sm" />
              ...
              <div className="flex gap-2 w-full sm:w-auto pt-1 sm:pt-0">
                ...
                {server.social_links && server.social_links.map((link, idx) => {
                  const icons: Record<string, React.ReactNode> = {
                    ...
                    github: <SiGithub className="w-4 h-4 md:w-5 h-5" />,
                    x: <SiX className="w-4 h-4 md:w-5 h-5" />,
                    patreon: <SiPatreon className="w-4 h-4 md:w-5 h-5" />,
                    kofi: <SiKofi className="w-4 h-4 md:w-5 h-5" />,
              ...
              <div className="flex items-center gap-1.5 w-full md:w-auto justify-center md:justify-end">
                ...
                {user ? (
                  viewMode === 'voting' ? (
                    ...
                    <div className="flex flex-col gap-2 w-full md:w-auto mt-2 md:mt-0">
                      ...
                      <div className="relative w-full">
                        <PlayerAvatar 
                          username={debouncedMcUsername || 'MHF_Steve'}
                          alt="Minecraft Head" 
                  ...
                  <motion.button 
                    ...
                    className="bg-[#5865F2] text-white px-3 py-1.5 sm:px-6 sm:py-2 rounded font-headline font-bold text-[10px] sm:text-sm flex items-center justify-center gap-2 w-full md:w-auto"
                  >
          ...
          {(server.ip_or_code || server.bedrock_ip) && server.type === 'server' && (
            <div className="w-full bg-zinc-900/50 border border-zinc-800 p-5 md:p-6 rounded-xl">
              <h3 className="font-headline font-bold text-zinc-500 text-sm md:text-base mb-8 pb-4 border-b border-zinc-800/50 flex items-center gap-2 relative z-0">
                <div className="absolute left-0 bottom-0 -top-5 md:-top-6 -right-5 md:-right-6 bg-gradient-to-r from-transparent via-transparent to-realm-green/10 -z-10 rounded-tr-xl" />
              ...
              {isLiveLoading ? (
                <div className="space-y-3 animate-pulse">
                  ...
                  <div className="h-12 w-full bg-zinc-800/50 rounded-md border border-zinc-800/50" />
                  <div className="h-[70px] w-full bg-zinc-800/30 rounded-md" />
                </div>
              ) : isLiveError || !liveStatus?.online ? (
                ...
                <div className="space-y-3">
                  ...
                  <Suspense fallback={<div className="h-[90px] w-full animate-pulse bg-zinc-800/30 rounded-md mt-2" />}>
                    <ServerPlayerChart livePlayers={liveStatus.players} playerHistory={playerHistory} />
                  </Suspense>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5 md:gap-2">
              {currentRank !== undefined && currentRank !== null && (
                <div className="flex justify-between items-center">
                  ...
                  <Link to="/leaderboards" className="text-[#4EC44E] font-bold text-xs md:text-sm hover:underline hover:text-[#5cd45c] transition-colors">#{currentRank}</Link>
                </div>
              )}
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-1.5 text-zinc-400">
              ...
              {(owner || (staff && staff.length > 0)) && (
                <div className="pt-4 mt-1 border-t border-zinc-800/50 space-y-3">
                  {owner && (
                    <Link 
                      to={`/profile/${owner.discord_username}`}
                      className="flex items-center gap-3 md:gap-4 -mx-5 md:-mx-6 px-5 md:px-6 group transition-all duration-300"
                    >
                      <img src={owner.discord_avatar || 'https://cdn.discordapp.com/embed/avatars/0.png'} className={`w-8 h-8 md:w-10 md:h-10 rounded-full border bg-zinc-800 object-cover ${isPremium ? 'border-[#f2a929]' : 'border-zinc-700'}`} alt="Owner" />
                      <div className="flex-1 min-w-0">
                        <p className="text-white text-xs md:text-sm leading-tight truncate flex items-center gap-1.5">
                      ...
                      return (
                        ...
                        >
                          <img 
                            ...
                            className={`w-8 h-8 md:w-10 md:h-10 rounded-full border bg-zinc-800 object-cover ${isStaffPremium ? 'border-[#f2a929]' : 'border-zinc-700'}`} 
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className={`text-[8px] md:text-[9px] uppercase tracking-widest font-headline mt-0.5 ${
                              isStaffPremium ? 'text-yellow-400' : member.role_title === 'Admin' ? 'text-realm-green' : 'text-zinc-500'
                            }`}>
                              {member.role_title}
                            </p>
                          </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                {badges.map((badge, idx) => (
                  ...
                  >
                    ...
                    {/* Premium Tooltip */}
                    <div className={`absolute bottom-full mb-3 w-48 opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none z-50 translate-y-2 group-hover:translate-y-0 ${
                      idx === 0 
                      ...
                      <div className={`w-2 h-2 bg-zinc-950 border-r border-b border-white/10 rotate-45 -mt-1 relative z-10 ${
                        idx === 0 
                          ? 'ml-3 sm:mx-auto' 
                          : 'mx-auto'
                      }`} />
        ...
        <div className="w-full bg-zinc-900/50 border border-zinc-800 p-5 md:p-8 rounded-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
            {ratings && ratings.length > 0 ? (
              <>
                {ratings.map((rating) => {
                  const isOwnReview = Boolean(user && rating.user_id === user.id)
                  return (
                    <motion.div 
                      ...
                      viewport={{ once: true }}
                      key={rating.id} 
                      className="bg-zinc-950 border border-zinc-800 p-5 md:p-6 rounded-xl space-y-3 relative min-h-[90px]"
                    >
                      <div className="flex justify-between items-start">
                        <Link 
                          to={`/profile/${rating.profiles?.discord_username || rating.profiles?.id}`} 
                          className="flex items-center gap-2 group/reviewer cursor-pointer"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <img 
                            src={rating.profiles?.discord_avatar || `https://ui-avatars.com/api/?name=${rating.profiles?.discord_username || 'Anonymous'}&background=random`} 
                            alt="Avatar" 
                            className="w-7 h-7 md:w-8 md:h-8 rounded-full border border-zinc-700 bg-zinc-900"
                          />
                          <div>
                      ...
                      {rating.comment && (
                        <p className={`text-zinc-400 text-xs md:text-sm leading-relaxed font-body italic ${isOwnReview ? 'pr-8' : ''}`}>" {rating.comment} "</p>
                      )}

                      {isOwnReview && (
                        <button
                          onClick={() => setIsRatingModalOpen(true)}
                          className="absolute bottom-3 right-3 md:bottom-4 md:right-4 p-1.5 text-zinc-500 hover:text-white hover:bg-zinc-900 border border-zinc-800/80 hover:border-zinc-700 rounded-md transition-all flex items-center justify-center group/edit shadow-sm"
                          title="Edit Review"
                          aria-label="Edit Review"
                        >
                          <Pencil className="w-3.5 h-3.5 group-hover/edit:text-realm-green transition-colors" />
                        </button>
                      )}
                    </motion.div>
                  )
                ...
                {/* Call to action item: only show if user hasn't rated yet */}
                {(!user || !ratings.some((r: any) => r.user_id === user.id)) && (
                  <motion.div 
                    initial={{ opacity: 0, x: -20 }}
                    whileInView={{ opacity: 1, x: 0 }}
                    viewport={{ once: true }}
                    onClick={() => {
                      if (!user) {
                        toast.error('Login Required', { description: 'You must be logged in to review a listing.' })
                        return
                      }
                      setIsRatingModalOpen(true)
                    }}
                    className="bg-zinc-950/50 border border-dashed border-zinc-700/50 p-5 md:p-6 rounded-xl flex flex-col items-center justify-center text-center cursor-pointer hover:bg-zinc-900/50 hover:border-realm-green/50 transition-colors group min-h-[120px]"
                  >
                    <Star className="w-6 h-6 text-zinc-600 group-hover:text-realm-green mb-3 transition-colors" />
                    <p className="text-zinc-400 font-headline text-[10px] md:text-xs tracking-wide group-hover:text-zinc-300 transition-colors leading-relaxed">
                      Thoughts about this server? Rate now!
                    </p>
                  </motion.div>
                )}
              </>
              ...
              <div 
                ...
                className="md:col-span-2 py-8 md:py-12 text-center cursor-pointer group hover:bg-zinc-900/30 rounded-xl transition-colors border border-transparent hover:border-zinc-800/50"
              >
                <Star className="w-8 h-8 text-zinc-700 group-hover:text-realm-green mb-3 mx-auto transition-colors" />
                <p className="text-zinc-500 font-headline text-[10px] md:text-sm uppercase tracking-widest group-hover:text-zinc-300 transition-colors">No reviews yet. Be the first to rate!</p>
              </div>
            )}
          </div>
        </div>
                    ...
                    return (
                      <div key={rank} className="flex items-center justify-between bg-zinc-950 border border-zinc-800/50 p-2.5 rounded-md hover:border-zinc-700 transition-colors shadow-sm">
                        <div className="flex items-center gap-2.5">
                          ...
                          <PlayerAvatar username={voter.minecraft_username} className="w-6 h-6 rounded-sm bg-zinc-900 border border-zinc-800" alt={voter.minecraft_username} />
