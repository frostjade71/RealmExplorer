      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 py-8 md:py-24 space-y-10 md:space-y-32">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 max-w-6xl mx-auto">
            {[
              { type: 'realm', label: 'Realm OTM', shadow: 'hover:shadow-purple-500/20 hover:border-purple-500/50', iconColor: 'text-purple-400', textColor: 'text-purple-400', defaultLink: '/rotm' },
              { type: 'server', label: 'Server OTM', shadow: 'hover:shadow-realm-green/20 hover:border-realm-green/50', iconColor: 'text-realm-green', textColor: 'text-realm-green', defaultLink: '/sotm' },
              { type: 'builder', label: 'Builder OTM', shadow: 'hover:shadow-orange-500/20 hover:border-orange-500/50', iconColor: 'text-orange-400', textColor: 'text-orange-400', defaultLink: '/botm' },
              { type: 'developer', label: 'Developer OTM', shadow: 'hover:shadow-blue-500/20 hover:border-blue-500/50', iconColor: 'text-blue-400', textColor: 'text-blue-400', defaultLink: '/dotm' },
            ].map((award, i) => {
              const winner = winners?.find(w => w.category === award.type)
              const link = winner
                ? (award.type === 'realm' || award.type === 'server'
                  ? `/server/${winner.winner_slug || winner.servers?.slug}`
                  : `/profile/${winner.profiles?.discord_username || winner.winner_slug}`)
                : award.defaultLink

              return (
                <Link to={link} key={i} className="group flex flex-col">
                  <div className={`relative w-full aspect-[4/5] bg-zinc-900/40 border border-white/5 rounded-lg transition-all duration-300 hover:-translate-y-1 hover:bg-zinc-900/70 shadow-lg ${award.shadow} overflow-hidden flex flex-col justify-between p-3 md:p-6`}>
                    ...
                    <div className="relative z-10 flex flex-col items-center justify-center text-center my-auto w-full">
                      ...
                      <h3 className="text-white font-pixel text-xs md:text-lg mb-1 truncate w-full drop-shadow-md">
                        {winner?.winner_name || (award.type.charAt(0).toUpperCase() + award.type.slice(1))}
                      </h3>
                    ...
                    <div className="relative z-10 flex flex-col items-center gap-1.5 mt-auto w-full">
                      ...
                      {winner?.month && (
                        <span className="text-[8px] md:text-[10px] font-pixel text-white uppercase tracking-wide">
                          {winner.month}
                        </span>
                      )}
                    </div>
